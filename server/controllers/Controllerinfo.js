...
module.exports.gethotelrooms = function(req,res) {
  ...
  connection.query('SELECT * FROM rooms WHERE rno NOT IN (SELECT rno FROM hotelbook WHERE hid = ? AND (checkin <= ? OR checkout >= ?))',[hid, checkout, checkin],function(err,result){
    if(err)
    {
      console.log(err);
      res.send({success: false});
...
module.exports.userfeed = function(req, res) {
  ...
  connection.query('SELECT id from login where username = ? AND type = ?', [username,"user"], function(err, result) {
    if(err) {
      ...
      var userid = result[0].id;
      connection.query("SELECT * FROM hotelbook t1 INNER JOIN hotel t2 ON t1.hid = t2.hid INNER JOIN rooms t3 ON t1.rno = t3.rno AND t1.hid = t3.hid WHERE t1.userid = ?", [userid], function(err, result) {
        if(err) {
          console.log(err);
